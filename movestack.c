...
movestack(const Arg *arg) {
	...
	if(arg->i > 0) {
		...
		for(c = selmon->sel->next; c && (!ISVISIBLE(c, c->mon) || c->isfloating); c = c->next);
		if(!c)
			for(c = selmon->cl->clients; c && (!ISVISIBLE(c, c->mon) || c->isfloating); c = c->next);

	}
	else {
		/* find the client before selmon->sel */
		for(i = selmon->cl->clients; i != selmon->sel; i = i->next)
			if(ISVISIBLE(i, c->mon) && !i->isfloating)
				c = i;
		if(!c)
			for(; i; i = i->next)
				if(ISVISIBLE(i, c->mon) && !i->isfloating)
