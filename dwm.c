...
void deck(Monitor *m) {
    ...
    for (i = my = 0, c = nexttiled(m->cl->clients, m); c; c = nexttiled(c->next, m), i++)
        if (i < m->nmaster) {
    ...
    for (s = m->cl->stack; s; s = s->snext) {
        if (!ISVISIBLE(s, m) || s->isfloating)
            continue;
        for (i = my = 0, c = nexttiled(m->cl->clients, m); c && c != s; c = nexttiled(c->next, m), i++)
            ;
        if (i < m->nmaster)
            continue;
...
void drawbar(Monitor *m) {
    ...
    if (m == selmon || 1) { /* status is only drawn on selected monitor */
        ...
        tw = (int)TEXTW(stext) - lrpad / 2 + 2; /* 2px right padding */
...
void focus(Client *c) {
    ...
    if (selmon->lt[selmon->sellt]->arrange == monocle || selmon->lt[selmon->sellt]->arrange == deck)
        arrangemon(selmon);
...
void monocle(Monitor *m) {
    ...
    for (c = m->cl->stack; c && (!ISVISIBLE(c, m) || c->isfloating); c = c->snext)
        ;
    if (c && !c->isfloating) {
...
void tile(Monitor *m) {
    ...
    for (i = 0, my = ty = m->gappoh * oe, c = nexttiled(m->cl->clients, m); c; c = nexttiled(c->next, m), i++)
        if (i < m->nmaster) {
            ...
            resize(c, m->wx + mw + m->gappov * oe, m->wy + ty, m->ww - mw - (2 * c->bw) - 2 * m->gappov * oe,
                   h - (2 * c->bw), 0);
...
void focusmaster(const Arg *arg) {
    ...
    c = nexttiled(selmon->cl->clients, selmon);
